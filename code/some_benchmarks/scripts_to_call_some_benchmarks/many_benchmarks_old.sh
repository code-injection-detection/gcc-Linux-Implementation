...
if [[ ( "$SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16" -eq 1 ) ]]; then
	sed -i 's/SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16=0/SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16=1/' ${ORIGINAL_DIR}/backup_automate.sh
fi

if [[ ( "$SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16" -eq 0 ) ]]; then
...
if [[ ( "$SPLIT_THE_BLOCKS_WHEN_THE_SECURE_CPU_WOULD" -eq 0 ) ]]; then
	sed -i 's/SPLIT_THE_BLOCKS_WHEN_THE_SECURE_CPU_WOULD=1/SPLIT_THE_BLOCKS_WHEN_THE_SECURE_CPU_WOULD=0/' ${ORIGINAL_DIR}/backup_automate.sh
fi
...
if [[ ( "$CALC_TIME_WITH_SEPARATE_MAC_ADDITION" -eq 1) && ( "$RUN_SECURE_EXEC" -eq 1 ) ]]; then #calculate proper mac times
	...
	if [[ ( "$SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16" -eq 1 ) ]]; then
		sed -i 's/SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16=0/SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16=1/' automate.sh
	fi
	if [[ ( "$SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16" -eq 0 ) ]]; then
		sed -i 's/SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16=1/SET_AS_GIVEN_THAT_EVERYTHING_MACED_WILL_BE_FIXED_AND_MULTIPLE_OF_16=0/' automate.sh
	fi
	echo "Calculating proper mac times..."
	./automate.sh 32 20 3 16 70000 16 20000 16 16 25 >/dev/null
...
do_benchmarks() {
	...
	VARIABLE_CODE_SIZE_NUMBERS=$2
	CACHE_SIZES=$3
	CODE_AND_DATA_CACHE_ASSOCS=$4
	DATA_BLOCK_SIZE_AS_CODE_BLOCK=$5
	DO_CACHES_ONLY=$6
	CODE_CACHE_TYPE=$7
	DATA_CACHE_TYPE=$8
	
	#echo ${FIXED_CODE_SIZE_NUMBERS}
	#echo ${VARIABLE_CODE_SIZE_NUMBERS}
	#echo ${CACHE_SIZES}
	#echo ${CODE_AND_DATA_CACHE_ASSOCS}
	...
	if [[ ( "$SPLIT_THE_BLOCKS_WHEN_THE_SECURE_CPU_WOULD" -eq 1 ) ]]; then
		sed -i 's/SPLIT_THE_BLOCKS_WHEN_THE_SECURE_CPU_WOULD=0/SPLIT_THE_BLOCKS_WHEN_THE_SECURE_CPU_WOULD=1/' ${ORIGINAL_DIR}/automate_template.sh
	fi
		...
		for var_size in ${VARIABLE_CODE_SIZE_NUMBERS}; do
			...
			if [[ ( "$DO_CACHES_ONLY" -eq 0 ) ]]; then
				...
				if [[ ( "$PRODUCE_SECURE_EXEC" -eq 1 ) ]]; then
					./automate.sh 32 ${var_size} 3 ${DATA_BLOCK_SIZE} ${SECURE_HEAP_SIZE} ${DATA_BLOCK_SIZE} ${SECURE_STACK_SIZE} ${DATA_BLOCK_SIZE} 0 >/dev/null
			...
			if [[ ( "$DO_CACHES_ONLY" -eq 0 ) ]]; then
				...
				sed -i 's/ADD_CODE_ON_THE_FLY_VERIFICATION=1/ADD_CODE_ON_THE_FLY_VERIFICATION=0/' automate.sh
				if [[ ( "$PRODUCE_SECURE_EXEC" -eq 1 ) ]]; then
					./automate.sh 32 ${var_size} 3 ${DATA_BLOCK_SIZE} ${SECURE_HEAP_SIZE} ${DATA_BLOCK_SIZE} ${SECURE_STACK_SIZE} ${DATA_BLOCK_SIZE} 16 >/dev/null
					cp ./main_program_ksec ${NAME_OF_EXEC}
					cp heap_keyshares ${NAME_OF_HEAP_KEYSHARES}
					cp stack_keyshares ${NAME_OF_STACK_KEYSHARES}
					cp all_keyshares_for_verification ${NAME_OF_ALL_KEYSHARES_FOR_VERIF}
	...
	for FIXED_SIZE in ${FIXED_CODE_SIZE_NUMBERS}; do
		...
		if [[ ( "$DATA_BLOCK_SIZE_AS_CODE_BLOCK" -eq 1 ) ]]; then
			DATA_BLOCK_SIZE=$(( ${FIXED_SIZE}-9 )) #9 is the number of the added code (verification + jmps)
		fi
		...
		if [[ ( "$DO_CACHES_ONLY" -eq 0 ) ]]; then
			...
			sed -i 's/ADD_CODE_ON_THE_FLY_VERIFICATION=1/ADD_CODE_ON_THE_FLY_VERIFICATION=0/' automate.sh
			if [[ ( "$PRODUCE_SECURE_EXEC" -eq 1 ) ]]; then
				./automate.sh 32 ${MAX_NUM_OF_CMDS_IN_FIXED} 3 ${DATA_BLOCK_SIZE} ${SECURE_HEAP_SIZE} ${DATA_BLOCK_SIZE} ${SECURE_STACK_SIZE} ${DATA_BLOCK_SIZE} 0 ${FIXED_SIZE} >/dev/null
				cp ./main_program_ksec ${NAME_OF_EXEC}
				cp heap_keyshares ${NAME_OF_HEAP_KEYSHARES}
				cp stack_keyshares ${NAME_OF_STACK_KEYSHARES}
				cp all_keyshares_for_verification ${NAME_OF_ALL_KEYSHARES_FOR_VERIF}
		...
		if [[ ( "$DO_CACHES_ONLY" -eq 0 ) ]]; then
			...
			sed -i 's/ADD_CODE_ON_THE_FLY_VERIFICATION=1/ADD_CODE_ON_THE_FLY_VERIFICATION=0/' automate.sh
			if [[ ( "$PRODUCE_SECURE_EXEC" -eq 1 ) ]]; then
				./automate.sh 32 ${MAX_NUM_OF_CMDS_IN_FIXED} 3 ${DATA_BLOCK_SIZE} ${SECURE_HEAP_SIZE} ${DATA_BLOCK_SIZE} ${SECURE_STACK_SIZE} ${DATA_BLOCK_SIZE} 16 ${FIXED_SIZE} >/dev/null
				cp ./main_program_ksec ${NAME_OF_EXEC}
				cp heap_keyshares ${NAME_OF_HEAP_KEYSHARES}
				cp stack_keyshares ${NAME_OF_STACK_KEYSHARES}
				cp all_keyshares_for_verification ${NAME_OF_ALL_KEYSHARES_FOR_VERIF}
...
FIXED_CODE_SIZE_NUMBERS="25 41 57 73" #-9 is multiple of 16
...
CODE_CACHE_TYPE=2
DATA_CACHE_TYPE=2
do_benchmarks "${FIXED_CODE_SIZE_NUMBERS}" "${VARIABLE_CODE_SIZE_NUMBERS}" "${CACHE_SIZES}" "${CODE_AND_DATA_CACHE_ASSOCS}" ${DATA_BLOCK_EQUAL_TO_CODE_BLOCK} 0 ${CODE_CACHE_TYPE} ${DATA_CACHE_TYPE}
CACHE_SIZES="1 2 4 6 8 10"
CODE_AND_DATA_CACHE_ASSOCS="DIRECT_MAPPED"
CODE_CACHE_TYPE=1
DATA_CACHE_TYPE=1
do_benchmarks "${FIXED_CODE_SIZE_NUMBERS}" "${VARIABLE_CODE_SIZE_NUMBERS}" "${CACHE_SIZES}" "${CODE_AND_DATA_CACHE_ASSOCS}" ${DATA_BLOCK_EQUAL_TO_CODE_BLOCK} 1 ${CODE_CACHE_TYPE} ${DATA_CACHE_TYPE}
