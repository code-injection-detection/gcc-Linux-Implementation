...
/*
gcc longer_nonlinear_least_squares_third.c -lgsl -lgslcblas -lm
*/

#define PRINT_RES 0
#define MAX_ITER 300 //orig:200
#define x_tol 1.0e-9 //orig:1.0e-8
#define g_tol 1.0e-9 //orig:1.0e-8
#define f_tol 1.0e-9 //orig:1.0e-8
#define DATA_POINTS 300000 //orig:300
             ...
             gsl_multifit_nlinear_parameters *params)
{
  ...
  const size_t max_iter = MAX_ITER;
  const double xtol = x_tol;
  const double gtol = g_tol;
  const double ftol = f_tol;
...
main (void)
{
  const size_t n = DATA_POINTS;  /* number of data points to fit */
