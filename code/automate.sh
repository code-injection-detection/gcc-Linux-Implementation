...
BYTES_FOR_INSTR_LEN=1 #the number of bytes after the canaries that hold the size of the verif+useful bytes+the jmp bytes
OVERHEAD_OF_VERIFICATION=7 #7 for fixed size, 14 for variable size (obsolete)
USING_LARGE_JMPS_AND_CODE_BLOCKS_WITH_3_WORLDS=0 #using the new implementation which restricts the variables that we have. The worlds are: no macs, ignoring macs, splitting and caching like secure cpu would
...
if [ "$BYTES_FOR_INSTR_LEN" == "1" ]; then
	...
	java -cp ../bin Secure_Assembly_new $NUM_OF_INTERLEAVED_KEYS $NUM_OF_GROUPED_INSTRUCTIONS $NUM_OF_CANARIES $NUM_OF_MAC_BYTES $ADD_CODE_ON_THE_FLY_VERIFICATION $NUM_OF_BYTES_IN_CODE_CHUNK $ramtmp $IGNORE_MACS_EVEN_IF_THERE_ARE_MAC_BYTES $SPLIT_THE_BLOCKS_WHEN_THE_SECURE_CPU_WOULD
fi
if [ $? -eq 0 ]; then
		: #all ok
	else
