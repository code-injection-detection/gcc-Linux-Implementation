...
USE_STACK_CANARIES=0 # IMPORTANT: ONLY WORKS WHEN STACK_SHOULD_GROW_TO_DECREASING_NUMBERS=1. Inserts a stack canary on top of the local variables of a function, in order to protect from buffer overflows. 1->use constant static stack canaries, 2-> use stack canaries different for every block in the secure stack , 3->use cryptographically distinct stack canaries (not yet supported)
