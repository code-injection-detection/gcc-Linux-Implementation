...
ALL_GETTER_SETTER_ARGUMENTS_POINT_AT_START_OF_BLOCKS=0 #Default 0. A pointer to the secure memory given to a getter or a setter may not be at the start of a block. Applies to heap/stack only, since we have GROUP_AS_MANY_GLOBAL_VARS_AS_POSSIBLE_IN_EACH_BLOCK for the globals
