...
void stack_fun_params_test()
{
	...
	test_params=init_function_params(1,0,5L,'c','d','e','f','g',2L,42,41,0L,0L,2L,42.42,56.56,1L,100L,424242,1L,10L*sizeof(double),double_array);
	...
	set_stack_pointer_array_element(test_params->non_array_params->pointer_params,0,test_params->non_array_params->int_params);
	set_stack_int_array_element(get_stack_pointer_array_element(test_params->non_array_params->pointer_params,0),0,45);
	printf("Printing ints\n");
	for (i=0;i<2;i++)
		printf("%d ",get_stack_int_array_element(test_params->non_array_params->int_params,i));
	printf("\n");
	...
	for (i=0;i<10;i++)
	{
		printf("i=%d, %lf ",i,get_stack_double_array_element(test_params->non_array_params->arb_pointer_params[0],i));
	}
	
	printf("Fetching double array using arbitrary block\n");
	double_array=error_checking_malloc(10*sizeof(double),__func__,__LINE__);
	get_arbitrary_block_in_stack(10*sizeof(double),test_params->non_array_params->arb_pointer_params[0],double_array);
	for (i=0;i<10;i++)
	{
